...
def init_layers(nn_arch, seed=42):
...
def get_cost_value(Y_hat, Y):
    m = Y_hat.shape[1]
    first_term = np.dot(Y, np.log(Y_hat).T)
    second_term = np.dot(1 - Y, np.log(1 - Y_hat).T)
...
def single_layer_backward_propagation(
    dA_curr, W_curr, b_curr, Z_curr, A_prev, activation="relu"
):
    # number of examples
    m = A_prev.shape[1]
...
Y_test_hat, _ = full_forward_propagation(np.transpose(X_test), params_values, nn_arch)
